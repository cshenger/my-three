...
function createCorps() {
  ...
  const cubeMaterial = new THREE.MeshStandardMaterial({
    map: textureLoader.load("../../public/textures/stone/stone.jpg"),
    metalness: 0.2,
    roughness: 0.07,
  });

  const cubeMaterialWithBumpMap = cubeMaterial.clone();
  cubeMaterialWithBumpMap.bumpMap = textureLoader.load("../../public/textures/stone/stone-bump.jpg");

  const cube1 = addGeometryWithMaterial(scene, cube, "cube-1", gui, params, cubeMaterial);
  cube1.position.x = -17;
  cube1.rotation.y = (1 / 3) * Math.PI;

  const cube2 = addGeometryWithMaterial(scene, cube, "cube-2", gui, params, cubeMaterialWithBumpMap);
  cube2.position.x = 17;
